...
/*
Each of these turns a pointer into a small index, in place.  Write the whole
pointer-sized slot, not just its first four bytes: on a 64-bit build the top
half would otherwise keep the discarded pointer's high bits, and the callers
in WriteGEntities compare the result against sentinels like (gclient_t *)-2.
The saved image only ever keeps the low four bytes (see SG_Collapse below),
which is the entire value because indices are small and -1 / -2 sign-extend.
*/
...
*/
	...
	case F_BOOLPTR:
		// test the whole pointer, not just its first four bytes, so that a
		// 64-bit address whose low half happens to be zero is still "set"
		*(intptr_t *)pv = ( *(void **)pv != NULL );
		break;
...
32-bit save layout translation

Saved games store these structures as raw images, so a 64-bit build would
write files the retail (32-bit) executable cannot read, and vice versa.
The values themselves are architecture independent - every pointer has
already been turned into an index by EnumerateField - so only the layout
differs: pointer slots are 8 bytes instead of 4, and alignment padding
moves with them.

sg_layout.h holds, for each saved structure, the list of byte ranges that
map one layout onto the other.  It is generated by comparing full class
layouts dumped from 32-bit and 64-bit compiles of these same headers, so
it cannot drift from the structures by hand.  On a 32-bit build none of
this is compiled in and the structures are written directly.

=======================================================================
*/
#if defined(_WIN64) || defined(_M_X64) || defined(_M_ARM64) || defined(__LP64__) || defined(__x86_64__) || defined(__aarch64__)
#define SG_TRANSLATE_LAYOUT 1
#endif

#ifdef SG_TRANSLATE_LAYOUT
...
static qboolean SG_LayoutFor( unsigned long ulChid, int iLen64, const sgLayoutOp_t **ppOps, int *piNumOps, int *piSize32 )
{
	switch ( ulChid )
	{
	case 'GENT':
		if ( iLen64 != sgSize64_GENT ) break;
		*ppOps = sgLayout_GENT; *piNumOps = sizeof(sgLayout_GENT)/sizeof(sgLayout_GENT[0]); *piSize32 = sgSize32_GENT;
		return qtrue;
	case 'GCLI':
		if ( iLen64 != sgSize64_GCLI ) break;
		*ppOps = sgLayout_GCLI; *piNumOps = sizeof(sgLayout_GCLI)/sizeof(sgLayout_GCLI[0]); *piSize32 = sgSize32_GCLI;
		return qtrue;
	case 'GNPC':
		if ( iLen64 != sgSize64_GNPC ) break;
		*ppOps = sgLayout_GNPC; *piNumOps = sizeof(sgLayout_GNPC)/sizeof(sgLayout_GNPC[0]); *piSize32 = sgSize32_GNPC;
		return qtrue;
	case 'LVLC':
		// only written as far as LEVEL_LOCALS_T_SAVESTOP
		if ( iLen64 != sgSaveStop64_LVLC ) break;
		*ppOps = sgLayout_LVLC; *piNumOps = sizeof(sgLayout_LVLC)/sizeof(sgLayout_LVLC[0]); *piSize32 = sgSaveStop32_LVLC;
		return qtrue;
	default:
		break;
	}
...
// 64-bit image -> 32-bit image.  Pointer slots keep their low four bytes,
// which is the whole value because it is an index by this point.
static void SG_Collapse( const sgLayoutOp_t *pOps, int iNumOps, const byte *pbSrc, byte *pbDst, int iSize32 )
{
	memset( pbDst, 0, iSize32 );

	for ( int i = 0; i < iNumOps; i++ )
	{
		if ( pOps[i].off32 + pOps[i].len > iSize32 )
		{
			continue;	// past the save cutoff
		}
		memcpy( pbDst + pOps[i].off32, pbSrc + pOps[i].off64, pOps[i].len );
	}
}

// 32-bit image -> 64-bit image.  The destination is cleared first, so the
// upper half of every widened pointer slot ends up zero.
static void SG_Expand( const sgLayoutOp_t *pOps, int iNumOps, const byte *pbSrc, byte *pbDst, int iSize32, int iSize64 )
{
	memset( pbDst, 0, iSize64 );
	...
	for ( int i = 0; i < iNumOps; i++ )
	{
		...
		memcpy( pbDst + pOps[i].off64, pbSrc + pOps[i].off32, pOps[i].len );
	}
}

#endif	// SG_TRANSLATE_LAYOUT
...
#ifdef SG_TRANSLATE_LAYOUT
	{
		const sgLayoutOp_t	*pOps;
		int					iNumOps, iSize32;

		if ( SG_LayoutFor( ulChid, iLen, &pOps, &iNumOps, &iSize32 ) && iSize32 <= SG_LAYOUT_MAX_BYTES )
		{
			SG_Collapse( pOps, iNumOps, pbData, sgLayoutBuffer, iSize32 );
			gi.AppendToSaveGame( ulChid, sgLayoutBuffer, iSize32 );
		}
		else
		{
...
//	}
#ifdef SG_TRANSLATE_LAYOUT
	{
		...
		if ( SG_LayoutFor( ulChid, iSize, &pOps, &iNumOps, &iSize32 ) && iSize32 <= SG_LAYOUT_MAX_BYTES )
		{
			// the file holds the 32-bit image, so read that and widen it
			int iRead32 = gi.ReadFromSaveGame( ulChid, sgLayoutBuffer, bOkToSizeMisMatch ? 0 : iSize32 );

			if ( iRead32 != iSize32 )
			{
				// same allowance as below: a chunk from an older, smaller
				// version of the structure, zero-filled to the current size
				assert( ulChid == 'GCLI' );
				assert( iSize32 > iRead32 );
				if ( iRead32 < 0 || iRead32 > iSize32 )
				{
					G_Error( "EvaluateFields: chunk %c%c%c%c is %d bytes, expected %d", (char)(ulChid>>24),(char)(ulChid>>16),(char)(ulChid>>8),(char)ulChid, iRead32, iSize32 );
				}
				memset( &sgLayoutBuffer[iRead32], 0, iSize32 - iRead32 );
				gbSaveGameWasOldFormat = qtrue;
			}

			SG_Expand( pOps, iNumOps, sgLayoutBuffer, pbData, iSize32, iSize );
